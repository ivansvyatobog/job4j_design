    ...
    private static void validate(ArgsName argsName) {
        if (!Files.exists(Paths.get(argsName.get("path"))) || !argsName.get("path").endsWith(".csv")) {
            throw new IllegalArgumentException(
                    String.format("%s - not exist or file format is not correct", argsName.get("path")));
        }
        if (argsName.get("out").isEmpty() || !argsName.get("out").endsWith(".csv")) {
            throw new IllegalArgumentException(
                    String.format("%s - not exist or file format is not correct", argsName.get("out")));
        }
        if (argsName.get("filter").isBlank()) {
            throw new IllegalArgumentException(
                    String.format("%s - filter argument is empty!", argsName.get("filter")));
        }
    }
